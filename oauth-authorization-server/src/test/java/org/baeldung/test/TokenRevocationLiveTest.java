...
public class TokenRevocationLiveTest {
	...
	public void whenObtainingAccessToken_thenCorrect() {
		final Response authServerResponse = obtainAccessToken("fooClientIdPassword", "ThaAdmin", "ThaAdmin");
	...
	private Response obtainAccessToken(String clientId, String username, String password) {
		...
		return RestAssured.given().auth().preemptive().basic(clientId, "secret").and().with().params(params).when()
				.post("http://localhost:8081/oauth-authorization-server/oauth/token");
	...
	private void authorizeClient(String clientId) {
		...
		final Response response = RestAssured.given().auth().preemptive().basic(clientId, "secret").and().with()
				.params(params).when().post("http://localhost:8081/oauth-authorization-server/oauth/authorize");

	}

	// @Test
	// public void givenUser_whenRevokeToken_thenTokenInvalidError() {
	// final String accessToken1 = obtainAccessToken("fooClientIdPassword", "john",
	// "123");
	// final String accessToken2 = obtainAccessToken("fooClientIdPassword", "tom",
	// "111");
	// authorizeClient("fooClientIdPassword");
	//
	// final Response tokenResponse1 = RestAssured.given().header("Authorization",
	// "Bearer " +
	// accessToken2).get("http://localhost:8082/spring-security-oauth-resource/tokens");
	// assertEquals(200, tokenResponse1.getStatusCode());
	//
	// final Response revokeResponse = RestAssured.given().header("Authorization",
	// "Bearer " +
	// accessToken1).post("http://localhost:8082/spring-security-oauth-resource/tokens/revoke/"+accessToken2);
	// assertEquals(200, revokeResponse.getStatusCode());
	//
	// final Response tokenResponse2 = RestAssured.given().header("Authorization",
	// "Bearer " +
	// accessToken2).get("http://localhost:8082/spring-security-oauth-resource/tokens");
	...
	// RestAssured.given().header("Authorization", "Bearer " +
	// accessToken3).get("http://localhost:8082/spring-security-oauth-resource/tokens");
	// assertEquals(200, refreshTokenResponse.getStatusCode());
	//
	// final Response revokeRefreshTokenResponse =
	// RestAssured.given().header("Authorization", "Bearer " +
	// accessToken1).post("http://localhost:8082/spring-security-oauth-resource/tokens/revokeRefreshToken/"+refreshToken);
	...
	// authorizeClient("fooClientIdPassword");
	// final Response refreshTokenResponse2 =
	// RestAssured.given().header("Authorization", "Bearer " +
	// accessToken4).get("http://localhost:8082/spring-security-oauth-resource/tokens");
